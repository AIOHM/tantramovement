...
class Module extends Module_Base {
	...
	public function register_controls( $element ) {
				...
				'groups'      => array(
					...
					'magic'    => array(
						...
						'options' => array(
							...
							'svg'       => __( 'SVG Draw', 'premium-addons-pro' ),
		...
		$repeater->add_control(
			'premium_mscroll_auto_rotate',
			array(
				'label'     => __( 'Auto Rotate', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SWITCHER,
				'condition' => array(
					'premium_mscroll_type' => 'custom',
				),
		...
		$repeater->add_control(
			'premium_mscroll_svg_note',
			array(
				'raw'             => __( 'SVG Path works only with paths generated from ', 'premium-addons-pro' ) . '<a href="https://codepen.io/GreenSock/full/oNNEdRV" target="_blank">here</a>. For accurate results, make sure to check on frontend.',
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition'       => array(
					'premium_mscroll_type' => 'custom',
		...
		$repeater->add_responsive_control(
			...
			array(
				...
				'selectors'   => array(
					'{{CURRENT_ITEM}}.premium-mscroll-svg' => 'width: {{SIZE}}%;',
				),
			)
		);
		...
		$repeater->add_control(
			...
			array(
				...
				'selectors'   => array(
					'{{CURRENT_ITEM}}.premium-mscroll-svg' => 'left: {{SIZE}}%;',
				),
			)
		);
		...
		$repeater->add_control(
			...
			array(
				...
				'selectors'   => array(
					'{{CURRENT_ITEM}}.premium-mscroll-svg' => 'top: {{SIZE}}%;',
				),
			)
		);
		...
		$element->add_control(
			...
			array(
				'raw'             => __( 'Perspective value is used with rotateX, rotateY, rotateZ effects to create 3D effects.', 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition'       => array(
					'premium_mscroll_switcher'           => 'yes',
		...
		$element->add_control(
			'premium_mscroll_refresh_note',
			array(
				'raw'             => __( 'The animation will reverse on scroll up when top of viewport hits the center of the parent container.', 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition'       => array(
					'premium_mscroll_switcher'     => 'yes',
					'premium_mscroll_anim_trigger' => 'automatic',
					'premium_mscroll_anim_rev'     => 'yes',
				),
			)
		);
	...
	private function add_helpful_information( $element ) {
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'mscroll-addon', 'wp-editor', 'get-support' );
	...
	private function register_responsive_controls( $element ) {
		...
		$element->add_control(
			'premium_mscroll_disable_notice',
			array(
				'raw'             => __( 'This option checks the height of the outer container, not any child containers in it.', 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'conditions'      => array(
					'relation' => 'or',
					'terms'    => array(
						array(
							'name'  => 'mscroll_desktop_action',
							'value' => 'height',
						),
						array(
							'name'  => 'mscroll_tablet_action',
							'value' => 'height',
						),
						array(
							'name'  => 'mscroll_mobile_action',
							'value' => 'height',
						),
					),
				),
	...
	public function before_render( $element ) {
		...
		if ( 'yes' === $element->get_settings_for_display( 'premium_mscroll_switcher' ) && isset( $settings['premium_mscroll_repeater'] ) ) {
