...
class Premium_Google_Reviews extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .premium-fb-rev-container .slick-arrow:hover' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .premium-fb-rev-container .slick-arrow:hover' => 'background-color: {{VALUE}};',
				),
			)
		);
	...
	protected function render() {
		...
		if ( false === $response ) {
			...
			/**
			 * Performance Optimization: Resolve and cache the Google Place avatar URL
			 * during the initial API fetch. This prevents redundant external HTTP requests
			 * on every subsequent widget render while the transient is valid.
			 */
			if ( 'old' === $version ) {
				if ( isset( $response->result ) ) {
					$image_url                   = premium_place_avatar( $response->result, $api_key );
					$response->result->image_url = str_replace( '/o.', '/ls.', $image_url );
				}
			} else {
				$image_url           = premium_place_avatar_new( $response, $api_key );
				$response->image_url = str_replace( '/o.', '/ls.', $image_url );
			}
