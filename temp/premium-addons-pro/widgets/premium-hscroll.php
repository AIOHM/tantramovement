...
class Premium_Hscroll extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
			$demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-horizontal-scroll-widget/', 'hscroll', 'wp-editor', 'demo' );
			Helper_Functions::add_templates_controls( $this, 'horizontal-scroll', $demo );
		}
		...
		$temp_repeater->add_control(
			'anchor_id',
			array(
				'label'       => __( 'Slide ID', 'premium-addons-pro' ),
				'type'        => Controls_Manager::TEXT,
				'description' => __( 'This ID is used to anchor your links to this slide. Check this <a href="https://premiumaddons.com/docs/how-to-anchor-elementor-horizontal-scroll-slides/" target="_blank">tutorial</a>', 'premium-addons-pro' ),
		...
		$temp_repeater->add_control(
			'section_id_notice',
			array(
				'raw'             => __( 'Please note that floating/scroll effects added for the inner elements will work on the frontend only.', 'premium-addons-for-elementor' ),
		...
		$this->add_control(
			'force_snap_viewport',
			array(
				'label'        => __( 'Force Scroll to First Slide on Scroll', 'premium-addons-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'true',
				'default'      => 'true',
				'condition'    => array(
		...
		$this->add_responsive_control(
			...
			array(
				'label'     => __( 'Progress Bar Left Position (PX)', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SLIDER,
				'range'     => array(
					'px' => array(
						'min' => 0,
		...
		$this->add_responsive_control(
			...
			array(
				'label'     => __( 'Progress Bar Bottom Position (PX)', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SLIDER,
				'range'     => array(
					'px' => array(
						'min' => 0,
		...
		$this->add_control(
			...
			array(
				...
				'description'  => __( 'Enable or disable scrolling slides using Keyboard. It\'s recommend to test this on frontend, not editor.', 'premium-addons-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'true',
				'default'      => 'true',
				'separator'    => 'before',
		...
		$this->add_control(
			'disable_on',
			array(
				'label'              => __( 'Disable Horizontal Scroll On', 'premium-addons-pro' ),
				'type'               => Controls_Manager::SELECT2,
				'default'            => array(),
				'options'            => array_merge(
					array(
		...
		$this->add_control(
			'override_columns',
			array(
				'label'        => __( 'Put Columns/Containers Next to Each Other', 'premium-addons-pro' ),
				'type'         => Controls_Manager::SWITCHER,
				'description'  => __( 'This option will force the columns to be positioned next to each other on small screens. Make sure the column/container width is set to 50% on small devices.' ),
		...
		$docs = array(
			'https://premiumaddons.com/docs/elementor-horizontal-scroll-widget-tutorial/' => __( 'Getting started »', 'premium-addons-pro' ),
			...
			'https://www.youtube.com/watch?v=JdMD7-on8gs' => __( 'Check the video tutorial »', 'premium-addons-pro' ),
		);

		$doc_index = 1;
		foreach ( $docs as $url => $title ) {

			$doc_url = Helper_Functions::get_campaign_link( $url, 'hscroll-widget', 'wp-editor', 'get-support' );
	...
	protected function render() {
		...
		if ( 'snap' === $settings['scroll_effect'] ) {
			...
			$hscroll_settings['force_snap'] = 'true' === $settings['force_snap_viewport'];
		...
		$hscroll_settings['disableSnap'] = intval( $disable_snap );
		...
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'spacer' ) ); ?>></div>
			...
			<div class="premium-hscroll-slider">
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'scroller_wrap' ) ); ?>>
					<div class="premium-hscroll-sections-wrap premium-addons__v-hidden" data-scroll-opacity="<?php echo esc_attr( $opacity ); ?>">
						<?php
						foreach ( $templates as $index => $section ) :
