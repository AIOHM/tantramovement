...
class Premium_Smart_Post_Listing extends Widget_Base {
	...
	private function add_posts_section_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'toggle'    => false,
				'selectors' => array(
					'{{WRAPPER}} .premium-smart-listing__posts-wrapper .premium-smart-listing__post-content-wrapper' => 'align-items:{{VALUE}}; text-align:{{VALUE}}; ',
					'body.rtl {{WRAPPER}} .premium-smart-listing__posts-wrapper .premium-smart-listing__post-content-wrapper .post-categories ' => 'justify-content:{{VALUE}}; ',
				),
			)
		);
	...
	private function add_carousel_section_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'prefix_class'       => 'premium-carousel-fade-',
				'frontend_available' => true,
			)
		);
	...
	private function add_pagination_section_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'default'            => 'num',
				'label_block'        => true,
				'prefix_class'       => 'premium-pagination-type-',
				'frontend_available' => true,
				'condition'          => array(
					'premium_blog_paging' => 'yes',
					'post_type_filter!'   => 'related',
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'            => array(
					'{{WRAPPER}} .premium-smart-listing__pagination-container' => 'justify-content: {{VALUE}}',
				),
			)
		);
	}
	...
	private function add_fitlers_style() {
		...
		$this->add_control(
			'wrapped_menu_heading',
			array(
				'label'     => __( 'Wrapped Menu', 'premium-addons-pro' ),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'custom' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-smart-listing__filter-tabs-menu-wrapper  ul' => 'gap: {{SIZE}}px;',
				),
				'condition'  => array(
					'wrap_tabs_sw' => 'yes',
				),
		...
		$this->add_responsive_control(
			...
			array(
				'label'      => __( 'Item Padding', 'premium-addons-pro' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'separator'  => 'before',
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
		...
		$this->add_responsive_control(
			...
			array(
				'label'      => __( 'Menu Padding', 'premium-addons-pro' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
	...
	protected function get_filter_tabs_markup( $filters ) {
		...
		$filter_tabs_num = isset( $settings['filter_tabs_num'] ) && '' !== $settings['filter_tabs_num'] ? (int) $settings['filter_tabs_num'] : 0;
		$visibleFilters  = $wrap_elements && ! empty( $settings['first_tab_label'] ) ? max( 0, $filter_tabs_num - 1 ) : $filter_tabs_num;
		$total_filters   = count( $filters );

		// Ensure we don't remove all filters - if visibleFilters is 0 or less, or >= total, show all filters without wrapping
		if ( ! $wrap_elements || $visibleFilters <= 0 || $visibleFilters >= $total_filters ) {
			$wrapped_elements = array();
		} else {
			$wrapped_elements = array_splice( $filters, $visibleFilters );
		}
					...
					<ul>
						...
						if ( ! empty( $wrapped_elements ) ) {
							foreach ( $wrapped_elements as $index => $filter ) {
								...
								if ( empty( $settings['first_tab_label'] ) && 0 === $index && 0 === $filter_tabs_num ) {
									$this->add_render_attribute( $key, 'class', 'active' );
								}
								?>
