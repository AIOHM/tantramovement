...
class Premium_Image_Accordion extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$accordion_repeater->add_control(
			'container_id_notice',
			array(
				'raw'             => __( 'Use this option to load content from a container on the current page (example: container-1). The container must be added to the page before the Image Accordion widget.', 'premium-addons-pro' ),
								...
								<?php if ( 'yes' === $item['content_switcher'] ) : ?>
									<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'content' ) ); ?>>
										<?php if ( 'template' === $item['content_type'] ) : ?>
											...
											elseif ( 'id' === $item['content_type'] ) :
												?>
												<div class="premium-accordion-content-wrap-container"
													data-container-id="<?php echo esc_attr( $item['container_id'] ); ?>">
												</div>
										<?php else : ?>
											<?php
