...
class Premium_Tabs extends Widget_Base {
	...
	protected function is_dynamic_content(): bool {
		...
		if ( ! empty( $items ) ) {
			foreach ( $items as $item ) {
				if ( 'text_editor' !== $item['premium_tabs_content'] ) {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$repeater->add_control(
			'container_id_notice',
			array(
				'raw'             => __( 'Use this option to load content from a container on the current page (example: container-1). The container must be added to the page before the Tabs widget.', 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					...
					'{{WRAPPER}} .premium-tabs li.premium-tabs-nav-list-item, {{WRAPPER}} .premium-tabs-style-tzoid .premium-tabs-nav .premium-tabs-nav-list.premium-tabs-horizontal li:first-child' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .premium-tabs .premium-tabs-nav-list-item:not(.tab-current):hover .premium-tab-title' => 'color: {{VALUE}}',
				),
			)
		);
										...
										<section id="section-<?php echo esc_attr( $section_style . '-' . $index . '-' . $this->get_id() ); ?>" class="premium-tabs-content-section">
											<div class="premium-tab-content <?php echo esc_attr( 'premium-con-lq__' . $settings['content_lq_effect'] ); ?>">
												...
												<?php if ( 'text_editor' === $tab['premium_tabs_content'] ) { ?>
													...
													<?php
												} elseif ( 'id' === $tab['premium_tabs_content'] ) {
													?>
														<div class="premium-tab-content-wrap-container"
															data-container-id="<?php echo esc_attr( $tab['container_id'] ); ?>">
														</div>
													<?php
												} else {
