...
class Premium_Charts extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->start_controls_section(
			'general_settings',
			array(
				'label' => __( 'Chart Settings', 'premium-addons-pro' ),
			)
		);

		if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
			$demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-charts-widget/', 'charts', 'wp-editor', 'demo' );
			Helper_Functions::add_templates_controls( $this, 'charts', $demo );
		}
		...
		$this->add_control(
			'hover_color',
			array(
				'label'   => __( 'Change Color on Hover', 'premium-addons-pro' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			)
		);
		...
		$this->add_control(
			'suffix',
			array(
				'label'              => __( 'Suffix Text', 'premium-addons-pro' ),
				'type'               => Controls_Manager::TEXT,
	...
	protected function render() {
		...
		if ( 'csv' === $data_source ) {
			...
			foreach ( $settings['y_axis_data'] as $column_data ) {
				...
				if ( 'yes' !== $settings['hover_color'] ) {
					$col_settings['hoverBackgroundColor'] = $background;
				}
